...
_CoSELayout.prototype.processEdgeList = function (gm, edges, layout) {
  ...
  if (this.options.idealEdgeLength != null) {
    if (edgeCount > 0) CoSEConstants.DEFAULT_EDGE_LENGTH = FDLayoutConstants.DEFAULT_EDGE_LENGTH = idealLengthTotal / edgeCount;else if (!isFn(this.options.idealEdgeLength)) // in case there is no edge, but option gives a value to use
